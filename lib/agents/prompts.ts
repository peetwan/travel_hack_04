...
5. \`days\`: build a day-by-day plan that respects the bases. \`stay_at\` is the gem_id of the base that night. \`is_transfer_day\` true on days where the user changes base. Most days should be at the same base — slow exploration, day-trips, downtime. **Every day must have morning, afternoon, AND evening_dinner.** Morning + afternoon are ~1 short sentence each with concrete activities (NOT generic "explore the area"). On transfer days, afternoon is usually the transfer itself ("scenic 2h drive to next base via the Pai canyon viewpoint").
